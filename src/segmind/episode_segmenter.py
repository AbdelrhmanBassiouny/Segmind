...
class EpisodeSegmenter(ABC):
    ...
    def run_event_detectors(self) -> None:
        ...
        while True:
            ...
            if closed_threads and self.logger.event_queue.unfinished_tasks == 0:
                break  # Exit when threads are closed AND queue is empty
    ...
    def start_contact_threads_for_object(
        self, obj: Body, event: Optional[ContactEvent] = None
    ) -> None:
        """
...
class AgentEpisodeSegmenter(EpisodeSegmenter):
    ...
    def start_tracking_threads_for_new_object_and_event(
        self, new_object: Body, event: Optional[ContactEvent] = None
    ):
