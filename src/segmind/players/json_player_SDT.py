...
class JSONPlayer(FilePlayer):
    ...
    def _remove_ignored_objects(self, objects_to_ignore: List[int]):
        ...
        self.data_frames = {
            frame_id: {
                obj_id: v
                for obj_id, v in objects_data.items()
    ...
    def get_objects_poses(
        ...
        for object_id, object_poses_data in objects_data.items():
            ...
            if obj_name not in self.world.get_object_names():
                ...
                objects_poses[obj] = self.apply_orientation_correction_to_object_pose(
                    obj, pose
                )
        return objects_poses

    def apply_orientation_correction_to_object_pose(
        self, obj: Body, obj_pose: TransformationMatrix
    ) -> TransformationMatrix:
        """
    ...
    def calculate_average_rotation(self):
        ...
        mean_rotation = np.mean(
            [q.to_rotation_matrix() for q in self.correction_quaternions.values()],
            axis=0,
        )
    ...
    def get_base_points_of_object(
        self, obj: Body, transform_points: bool = True
    ) -> np.ndarray:
        """
        ...
        if transform_points:
            # Use SEMDT Quaternion instead of pycram's quaternion_matrix
            base_points = np.dot(
                base_points, obj.get_orientation.to_rotation_matrix().T
            )
        return base_points
    ...
    def get_mesh_of_object(
        self, obj: Body, apply_scale: bool = True, apply_transform: bool = True
    ) -> Trimesh:
        """
        ...
        """
        return self.object_meshes.get(
            obj,
            self.load_scale_and_transform_mesh_of_object(
                obj, apply_scale, apply_transform
            ),
        )

    def load_scale_and_transform_mesh_of_object(
        self, obj: Body, apply_scale: bool = True, apply_transform: bool = True
    ) -> Geometry:
        """
    ...
    @staticmethod
    def get_base_points_from_mesh(
        mesh: Trimesh, threshold: float = 0.001, return_min_z: bool = False
    ...
    def calculate_plane_rotation_from_normal(u_norm: np.ndarray) -> np.ndarray:
        ...
        rotation_matrix = scipy.spatial.transform.Rotation.align_vectors(
            u_norm, vertical_axis
        )[0].as_matrix()

        return rotation_matrix

    def get_pose_and_transform_to_map_frame(
        self, object_pose: dict
    ...
    @staticmethod
    def get_pose_from_frame_object_data(
        object_data: dict,
        ...
        """
        position = (
            np.array(list(map(float, object_data["t"]))) / 1000
    ...
    def transform_pose_to_map_frame(
