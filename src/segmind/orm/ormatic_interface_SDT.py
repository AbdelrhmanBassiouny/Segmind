...
import segmind.datastructures.events_SDT
...
m_ContactPoint = mapper_registry.map_imperatively(semantic_digital_twin.world_description.world_entity.ContactPoint, t_ContactPoint, properties = dict(position_on_body_a=relationship('Vector3',foreign_keys=[t_ContactPoint.c.position_on_body_a_id]),
...
m_ContactPointsList = mapper_registry.map_imperatively(semantic_digital_twin.world_description.world_entity.ContactPointsList, t_ContactPointsList, properties = dict(points=relationship('ContactPoint',foreign_keys=[t_ContactPoint.c.contactpointslist_points_id])), polymorphic_on = "polymorphic_type", polymorphic_identity = "ContactPointsList")

m_VisualShape = mapper_registry.map_imperatively(semantic_digital_twin.world_description.shape_collection.ShapeCollection, t_VisualShape, properties = dict(rgba_color=relationship('Color',foreign_keys=[t_VisualShape.c.rgba_color_id]),
...
m_FrozenBody = mapper_registry.map_imperatively(semantic_digital_twin.world_description.world_entity.Body, t_FrozenBody, properties = dict(pose=relationship('PoseStamped',foreign_keys=[t_FrozenBody.c.pose_id]),
...
m_BoundingBox = mapper_registry.map_imperatively(semantic_digital_twin.world_description.geometry.BoundingBox, t_BoundingBox, polymorphic_on = "polymorphic_type", polymorphic_identity = "BoundingBox")

m_Vector3 = mapper_registry.map_imperatively(semantic_digital_twin.spatial_types.spatial_types.Vector3, t_Vector3, polymorphic_on = "polymorphic_type", polymorphic_identity = "Vector3")

m_Pose = mapper_registry.map_imperatively(semantic_digital_twin.spatial_types.spatial_types.TransformationMatrix, t_Pose, properties = dict(position=relationship('Vector3',foreign_keys=[t_Pose.c.position_id]),
...
m_LateralFriction = mapper_registry.map_imperatively(semantic_digital_twin.world_description.world_entity.LateralFriction, t_LateralFriction, properties = dict(lateral_friction_direction=relationship('Vector3',foreign_keys=[t_LateralFriction.c.lateral_friction_direction_id])))

m_Color = mapper_registry.map_imperatively(semantic_digital_twin.world_description.geometry.Color, t_Color, )

m_PoseStamped = mapper_registry.map_imperatively(semantic_digital_twin.spatial_types.spatial_types.TransformationMatrix, t_PoseStamped, properties = dict(pose=relationship('Pose',foreign_keys=[t_PoseStamped.c.pose_id]),
...
m_FrozenJoint = mapper_registry.map_imperatively(semantic_digital_twin.world_description.world_entity.Body, t_FrozenJoint, )

m_FrozenWorldState = mapper_registry.map_imperatively(semantic_digital_twin.world_description.world_state.WorldState, t_FrozenWorldState, properties = dict(objects=relationship('FrozenObject',foreign_keys=[t_FrozenObject.c.frozenworldstate_objects_id])))

m_Quaternion = mapper_registry.map_imperatively(semantic_digital_twin.spatial_types.spatial_types.Quaternion, t_Quaternion, )

m_Header = mapper_registry.map_imperatively(pycram.datastructures.pose.Header, t_Header, )

m_ClosestPoint = mapper_registry.map_imperatively(semantic_digital_twin.world_description.world_entity.ContactPoint, t_ClosestPoint, polymorphic_identity = "ClosestPoint", inherits = m_ContactPoint)

m_ClosestPointsList = mapper_registry.map_imperatively(semantic_digital_twin.world_description.world_entity.ContactPointsList, t_ClosestPointsList, polymorphic_identity = "ClosestPointsList", inherits = m_ContactPointsList)

m_BoxVisualShape = mapper_registry.map_imperatively(semantic_digital_twin.world_description.geometry.Box, t_BoxVisualShape, properties = dict(half_extents=relationship('Vector3',foreign_keys=[t_BoxVisualShape.c.half_extents_id])), polymorphic_identity = "BoxVisualShape", inherits = m_VisualShape)

m_PlaneVisualShape = mapper_registry.map_imperatively(semantic_digital_twin.world_description.geometry.Plane, t_PlaneVisualShape, properties = dict(normal=relationship('Vector3',foreign_keys=[t_PlaneVisualShape.c.normal_id])), polymorphic_identity = "PlaneVisualShape", inherits = m_VisualShape)

m_SphereVisualShape = mapper_registry.map_imperatively(semantic_digital_twin.world_description.geometry.Sphere, t_SphereVisualShape, polymorphic_identity = "SphereVisualShape", inherits = m_VisualShape)

m_MeshVisualShape = mapper_registry.map_imperatively(semantic_digital_twin.world_description.geometry.Mesh, t_MeshVisualShape, properties = dict(scale=relationship('Vector3',foreign_keys=[t_MeshVisualShape.c.scale_id])), polymorphic_identity = "MeshVisualShape", inherits = m_VisualShape)

m_CapsuleVisualShape = mapper_registry.map_imperatively(semantic_digital_twin.world_description.geometry.Capsule, t_CapsuleVisualShape, polymorphic_identity = "CapsuleVisualShape", inherits = m_VisualShape)
...
m_FrozenLink = mapper_registry.map_imperatively(semantic_digital_twin.world_description.world_entity.Body, t_FrozenLink, properties = dict(geometry=relationship('VisualShape',foreign_keys=[t_VisualShape.c.frozenlink_geometry_id])), polymorphic_identity = "FrozenLink", inherits = m_FrozenBody)

m_FrozenObject = mapper_registry.map_imperatively(semantic_digital_twin.world_description.world_entity.Body, t_FrozenObject, properties = dict(links=relationship('FrozenLink',foreign_keys=[t_FrozenLink.c.frozenobject_links_id]),
...
m_AxisAlignedBoundingBox = mapper_registry.map_imperatively(semantic_digital_twin.world_description.geometry.BoundingBox, t_AxisAlignedBoundingBox, polymorphic_identity = "AxisAlignedBoundingBox", inherits = m_BoundingBox)

m_RotatedBoundingBox = mapper_registry.map_imperatively(semantic_digital_twin.world_description.geometry.BoundingBox, t_RotatedBoundingBox, polymorphic_identity = "RotatedBoundingBox", inherits = m_BoundingBox)

m_Vector3Stamped = mapper_registry.map_imperatively(semantic_digital_twin.spatial_types.spatial_types.Vector3, t_Vector3Stamped, properties = dict(header=relationship('Header',foreign_keys=[t_Vector3Stamped.c.header_id])), polymorphic_identity = "Vector3Stamped", inherits = m_Vector3)

m_Transform = mapper_registry.map_imperatively(semantic_digital_twin.spatial_types.spatial_types.TransformationMatrix, t_Transform, polymorphic_identity = "Transform", inherits = m_Pose)

m_TransformStamped = mapper_registry.map_imperatively(semantic_digital_twin.spatial_types.spatial_types.TransformationMatrix, t_TransformStamped, properties = dict(pose=relationship('Transform',foreign_keys=[t_TransformStamped.c.pose_id])), polymorphic_identity = "TransformStamped", inherits = m_PoseStamped)

m_GraspPose = mapper_registry.map_imperatively(semantic_digital_twin.spatial_types.spatial_types.GraspPose, t_GraspPose, polymorphic_identity = "GraspPose", inherits = m_PoseStamped)

m_CylinderVisualShape = mapper_registry.map_imperatively(semantic_digital_twin.world_description.geometry.Cylinder, t_CylinderVisualShape, polymorphic_identity = "CylinderVisualShape", inherits = m_CapsuleVisualShape)
