...
class EventLogger:
    ...
    def get_latest_event_of_detector_for_object(
        self, detector_prefix: str, obj: Body
    ) -> Optional[Event]:
        """
    ...
    def get_nearest_event_of_detector_for_object(
        self, detector_prefix: str, obj: Body, timestamp: float
    ) -> Optional[EventUnion]:
        """
...
class EventAnnotationThread(threading.Thread):
    def __init__(self, logger: EventLogger):
        super().__init__()
        self.logger = logger
        self.current_annotations: List[SemanticAnnotation] = []
    ...
    def run(self):
        while not self.kill_event.is_set() or not self.logger.annotation_queue.empty():
            ...
            try:
                ...
                logger.info(f"Error annotating event {event}: {e}")
                continue
