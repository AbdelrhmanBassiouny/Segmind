...
from segmind.detectors.atomic_event_detectors import (
    ...
    AtomicEventDetector,
    ContactDetector,
    DetectorWithTrackedObject,
    DetectorWithTwoTrackedObjects,
...
from segmind.detectors.coarse_event_detectors import (
    ...
    AbstractPickUpDetector,
    DetectorWithStarterEvent,
    DetectorWithTrackedObjectAndStarterEvent,
...
from segmind.episode_segmenter import (
    AgentEpisodeSegmenter,
    EpisodeSegmenter,
    NoAgentEpisodeSegmenter,
)
from ripple_down_rules.datastructures.case import Case
from semantic_digital_twin.world_description.world_entity import Body, Agent


def conditions_for_episode_segmenter_is_detector_redundant(
    self_: NoAgentEpisodeSegmenter,
    detector_type: Union[
    ...
    pick_up_detectors = [
        detector
        for (_, _), detector in self_.starter_event_to_detector_thread_map.items()
        if isinstance(detector, GeneralPickUpDetector)
    ]
    is_being_picked_objects = [
        detector.tracked_object for detector in pick_up_detectors if detector.is_alive()
    ]
    ...
    if latest_pick_up_event is not None:
        latest_placing_event = object_tracker.get_first_event_of_type_after_event(
            PlacingEvent, latest_pick_up_event
        )
        latest_insertion_event = object_tracker.get_first_event_of_type_after_event(
            InsertionEvent, latest_pick_up_event
        )
        if latest_placing_event is None and latest_insertion_event is None:
            return True
    picked_objects = [
        detector.tracked_object
        for detector in pick_up_detectors
        if not detector.is_alive()
    ]
    if starter_event.tracked_object in picked_objects:
        contact_points_list = getattr(
            starter_event.tracked_object, "contact_points", None
        )
