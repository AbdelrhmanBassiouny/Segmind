...
if TYPE_CHECKING:
    from .events_SDT import Event, EventUnion
    from ..detectors.coarse_event_detectors_SDT import DetectorWithStarterEvent
...
class ObjectTracker:
    ...
    def add_event(self, event: Event):
        ...
        if isinstance(self.obj, Body) and self.obj.parent_kinematic_structure_entity is not None:
            ObjectTrackerFactory.get_tracker(self.obj.parent_kinematic_structure_entity).add_event(event)
    ...
    def get_events_between_timestamps(self, timestamp1: float, timestamp2: float) -> List[Event]:
        with self._lock:
            ...
            try:
                ...
                logger.warning(f"No events between timestamps {timestamp1}, {timestamp2}")
                return []
