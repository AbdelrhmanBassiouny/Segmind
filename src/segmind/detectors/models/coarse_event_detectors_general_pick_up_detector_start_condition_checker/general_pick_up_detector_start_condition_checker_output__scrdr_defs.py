...
def conditions_185374571278911955422500089733244334209(case) -> bool:
    def does_the_support_spatially_contain_the_object(
        cls_: Type[GeneralPickUpDetector], event: Event, output_: bool
    ) -> bool:
        """Get conditions on whether it's possible to conclude a value for GeneralPickUpDetector_start_condition_checker.output_  of type ."""
        support = event.with_object
        return support.contains_body(
            event.tracked_object, update=True, intersection_ratio=0.9
        )
...
def conditions_259334668447470890319338447268411809573(case) -> bool:
    def is_the_support_a_virtual_hole(
        cls_: Type[GeneralPickUpDetector], event: Event, output_: bool
    ) -> bool:
        """Get conditions on whether it's possible to conclude a value for GeneralPickUpDetector_start_condition_checker.output_  of type ."""
...
def conditions_1084517150588802996811823834978763787(case) -> bool:
    def is_the_object_in_contact_with_a_virtual_hole(
        cls_: Type[GeneralPickUpDetector], event: Event, output_: bool
    ) -> bool:
        """Get conditions on whether it's possible to conclude a value for GeneralPickUpDetector_start_condition_checker.output_  of type ."""
        return any(
...
def conditions_257426441376249650265480473188288026956(case) -> bool:
    def did_the_object_free_fall(
        cls_: Type[GeneralPickUpDetector], event: Event, output_: bool
    ) -> bool:
        """Get conditions on whether it's possible to conclude a value for GeneralPickUpDetector_start_condition_checker.output_  of type ."""
        translation_event = event.object_tracker.get_first_event_of_type_before_event(
            TranslationEvent, event
        )
        if translation_event is None:
            return True
        if (
            event.tracked_object.pose.position.z
            - translation_event.start_pose.position.z
        ) >= (3 * 1e-3):
            return False
        else:
            return True
