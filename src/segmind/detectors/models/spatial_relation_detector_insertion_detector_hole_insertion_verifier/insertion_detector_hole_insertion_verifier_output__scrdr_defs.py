...
def conditions_313966059252436144481394373657043070884(case) -> bool:
    def conditions_for_insertion_detector_hole_insertion_verifier(
        ...
        if latest_insertion is None:
            return False
        latest_pickup = event.object_tracker.get_first_event_of_type_after_event(
            PickUpEvent, latest_insertion
        )
        no_pick_up = latest_pickup is None
        latest_insertion_before_this_insertion = (
            event.object_tracker.get_first_event_of_type_before_event(
                InsertionEvent, latest_insertion
            )
        )
        if latest_insertion_before_this_insertion is None:
            ...
            abs(
                latest_insertion.timestamp
                - latest_insertion_before_this_insertion.timestamp
            )
            < 0.5
        ):
            return True
