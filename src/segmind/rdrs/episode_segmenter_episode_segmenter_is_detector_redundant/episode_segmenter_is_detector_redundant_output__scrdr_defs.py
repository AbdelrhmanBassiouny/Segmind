...
def conditions_217503528191875472672592688900935027547(case) -> bool:
    def conditions_for_episode_segmenter_is_detector_redundant(
        ...
        is_being_picked_objects = [
            ...
            for detector in pick_up_detetectors
        ...
        if starter_event.tracked_object in picked_objects:
            contacted_bodies = (
                starter_event.tracked_object.contact_points.get_all_bodies()
            )
            ...
            agents = [
                ...
                if (
                    ...
                    and issubclass(body.parent_entity.obj_type, Agent)
                )
...
def conclusion_217503528191875472672592688900935027547(case) -> bool:
    def episode_segmenter_is_detector_redundant(
        self_: NoAgentEpisodeSegmenter,
        detector_type: Union[
            Type[ContactDetector],
            Type[LossOfContactDetector],
...
def conclusion_318535409151373315477142163500790537263(case) -> bool:
    def episode_segmenter_is_detector_redundant(
        ...
        similar_detectors = [
            (se, de, de_inst)
            for (se, de), de_inst in self_.starter_event_to_detector_thread_map.items()
        ...
        for prev_start_event, _, prev_detector in similar_detectors:
            ...
            new_pickable_objects = [
                ...
                if obj in prev_detector.currently_tracked_objects
            ]
            if len(new_pickable_objects) > 0:
                return False
            common_pickable_objects = [
                obj for obj in pickable_objects if obj not in new_pickable_objects
            ]
            for obj in common_pickable_objects:
                if (
                    starter_event.timestamp - prev_start_event.timestamp
                    < timedelta(milliseconds=200).total_seconds()
                ):
                    return True
        return False
