    ...
    for file in urdf_files:
        ...
        if obj_name == "iCub":
            ...
            pose = TransformationMatrix(
                TransformationMatrix.from_xyz_rpy(-0.8, 0, 0.55)
            )
        elif obj_name == "scene":
            obj_type = Region
        else:
...
multiverse_player = MultiversePlayer(
    world=world,
...
pickable_objects: Set[Body] = set()
latest_event: Optional[AbstractAgentObjectInteractionEvent] = None
all_inserted_objects: Set[Body] = set()
excluded_objects: List[Body] = [
    World.clear().get_object_by_name("montessori_object_1"),
    World.get_object_by_name("montessori_object_4"),
]
while True:
    pickable_objects = set(
        select_transportable_objects(World.current_world.objects, not_contained=True)
    )
    pickable_objects = {
        pobj for pobj in pickable_objects if pobj not in excluded_objects
    }
    pickable_objects = {
        obj for obj in pickable_objects if obj not in all_inserted_objects
    }
    ...
    if match_shapes and len(pickable_objects) > 0:
        ...
        actionable_events = [
            ...
            if isinstance(event, AbstractAgentObjectInteractionEvent)
            and not isinstance(event, PlacingEvent)
        ]
        actionable_events = sorted(actionable_events, key=lambda event: event.timestamp)
        if latest_event is not None:
            actionable_events = list(
                filter(
                    lambda e: e.timestamp > latest_event.timestamp, actionable_events
                )
            )
        ...
        pickable_objects = set(
            select_transportable_objects(
                World.current_world.objects, not_contained=True
            )
        )
        pickable_objects = {
            pobj for pobj in pickable_objects if pobj not in excluded_objects
        }
        all_inserted_objects = {
            event.tracked_object
            for event in actionable_events
            if isinstance(event, InsertionEvent)
        }
        pickable_objects = {
            obj for obj in pickable_objects if obj not in all_inserted_objects
        }
    action_descriptions: List[
        Tuple[
            Optional[AbstractAgentObjectInteractionEvent],
            PartialDesignator[ActionDescription],
        ]
    ] = []
    object_pick_up_actions: Dict[Body, PickUpActionDescription] = {}
    object_picked_arm_and_grasp: Dict[
        Body, Tuple[Arm, GraspDescription, TransformationMatrix]
    ] = {}
    mapped_objects: Dict[Body, Body] = {}
    loginfo(str(actionable_events))
    objects_to_insert = []

    for i, actionable_event in enumerate(actionable_events):
        action_descriptions.append(
            (actionable_event, actionable_event.action_description)
        )
        pickable_objects = set(
            select_transportable_objects(
                World.current_world.objects, not_contained=True
        ...
        pickable_objects = {
            obj for obj in pickable_objects if obj not in mapped_objects.values()
        }
        ...
        if isinstance(actionable_event, PickUpEvent):
            ...
            end_effector = RobotDescription.current_robot_description.get_arm_chain(
                arm
            ).end_effector
            ...
            action_to_add = (
                actionable_event,
                PickUpActionDescription(
                    to_pick_object, arm=arm, grasp_description=grasp
                ),
            )
            if len(action_descriptions) >= 1:
            ...
            if isinstance(actionable_event, PlacingEvent):
                ...
                if match_shapes:
                    ...
                    place_pose = scene_obj.links[
                        obj_hole_map[object_to_place.name]
                    ].pose
                else:
            ...
            action_descriptions.append((None, ParkArmsActionDescription(Arm.BOTH)))

    # episode_segmenter.reset()
    latest_event = sorted(
        episode_segmenter.logger.get_events(), key=lambda e: e.timestamp
    )[-1]
    ...
    with AbstractRobot:
        for event, partial_designator in action_descriptions:
            ...
            if issubclass(performable.action, PlaceAction):
                ...
                latest_pick_up = obj_tracker.get_first_event_of_type_after_event(
                    PickUpEvent, latest_event
                )
                if latest_pick_up is not None:
                    time.sleep(6)
                    insertion_event = obj_tracker.get_first_event_of_type_after_event(
                        InsertionEvent, latest_pick_up
                    )
                    if insertion_event is None:
                        ...
                        text_to_speech(
                            f"Hmmm, Looks like the {obj_name_map[obj.name]} was not inserted,"
                            f"Could you show me how or where to insert it?"
                        )
                        ParkArmsActionDescription(Arm.BOTH).current_node.perform()
                        break
                    else:
    ...
    while True:
        teacher_insertion_event: Optional[InsertionEvent] = (
            failed_insertion_object_tracker.get_first_event_of_type_after_event(
                InsertionEvent, latest_event
            )
        )
    ...
    failed_insertion_hole: Body = event_that_led_to_insertion_action.through_hole
    teacher_insertion_hole: Body = teacher_insertion_event.through_hole
    ...
    if type(obj_shape) is type(teacher_hole_shape) and type(obj_shape) is not type(
        failed_hole_shape
    ):
        match_shapes = True
        text_to_speech(
            "Ok thank you, I think I understand now what you want,"
            "you want me to match the object shape with the hole shape."
        )
    elif type(teacher_hole_shape) is type(failed_hole_shape) and type(
        obj_shape
    ) is type(failed_hole_shape):
        match_shapes = True
        if teacher_insertion_hole.name != failed_insertion_hole.name:
            size_diff = np.linalg.norm(
                np.array(teacher_hole_shape.size) - np.array(failed_hole_shape.size)
            )
            if size_diff >= 1e-3:
                teacher_difference = np.linalg.norm(
                    np.array(teacher_hole_shape.size) - np.array(obj_shape.size)
                )
                failed_difference = np.linalg.norm(
                    np.array(failed_hole_shape.size) - np.array(obj_shape.size)
                )
                if teacher_difference < failed_difference:
                    text_to_speech(
                        "Hmm ok so you chose a similar shaped hole like me, but you chose a different size,"
                        "might be because it is closer to the size of the object shape, Ok I see!"
                    )
                else:
                    text_to_speech(
                        "Hmm ok so you chose a similar shaped hole like me,"
                ...
                text_to_speech(
                    "Hmm, I didn't get why you chose this specific hole, but I guess it could be a matter of "
                    "preference from your side"
                )
        else:
            text_to_speech(
                "Hmm ok so you placed the object in the same hole I chose,"
        ...
        text_to_speech(
            "Hmm, I didn't get the idea, I do not see the patter here, sorry!"
        )
    ...
    if match_shapes:
        ...
        if len(pickable_objects) > 1:
            obj_names_to_insert = ", the".join(
                [obj_name_map[obj.name] for obj in list(pickable_objects)[:-1]]
            )
            obj_names_to_insert += (
            ...
            obj_names_to_insert = (
                obj_name + "has not been inserted yet, I can insert it if you let me."
            )
        else:
            continue
