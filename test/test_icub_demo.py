...
def set_up_demo_fixture(episode_name: str = "icub_montessori_no_hands"):
    ...
    world: World = World(WorldMode.DIRECT)
...
def copy_model_files_to_world_data_dir(models_dir):
    ...
    shutil.copytree(
        models_dir, World.current_world.conf.cache_dir + "/objects", dirs_exist_ok=True
    )
...
def test_icub_pick_up_and_insert(set_up_demo_fixture):
    ...
    obj_pose = obj.pose
    obj_pose = (
        World.current_world.get_object_by_name("scene").links["circular_hole_1"].pose
    )
    ...
    with real_robot:
        plan = SequentialPlan(
            ...
            PickUpActionDescription(
                object_description, arm=arm, grasp_description=grasp
            ),
            PlaceActionDescription(
                object_description, square_hole_pose, arm, insert=True
            ),
        )
        plan.perform()
