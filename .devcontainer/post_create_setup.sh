...
cd "$WORKSPACE_DIR/../Multiverse-Parser" && git pull && pip install -r "requirements.txt" && pip install -e .
cd "$WORKSPACE_DIR/../Multiverse-Resources" && git pull && git submodule update --init --recursive